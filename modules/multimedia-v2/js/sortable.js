...
// 			placeholder.style.height = box.height.toFixed()+"px";		
...
// function registerSortable(sortableManager) {
// 	var draggedElement, placeholder, offsetX, offsetY;
// 	function onMouseDown(event) {
// 		if (event.target.classList.contains("media-box-cell")) {
// 			event.preventDefault();
// 			draggedElement = event.target.parentNode;
// 			placeholder = tag("tr.media-box-row.placeholder", tag("td"), tag("td"), tag("td"), tag("td"));
// 			var box = draggedElement.getBoundingClientRect();
// 			offsetX = event.clientX - box.left;
// 			offsetY = event.clientY - box.top;
// 			draggedElement.parentNode.insertBefore(placeholder, draggedElement);
...
// 		var x = event.clientX - offsetX + draggedElement.clientWidth/2;
// 		var y = event.clientY - offsetY + draggedElement.clientHeight/2;
// 		sortableManager.test(draggedElement, x, y, function(dragFrom, dragTo) {
// 			
// // 			this.items.splice(draggedIndex, 1);
// // 					this.items.splice(index, 0, draggedElement);
// 			
// 			sortableManager.container.removeChild(placeholder);
// 			sortableManager.container.insertBefore(placeholder, sortableManager.container.children[dragTo]);
// 		});
// 	}
// 	function onMouseUp(event) {
// 		if (draggedElement && placeholder) {
